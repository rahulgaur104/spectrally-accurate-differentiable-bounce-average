...
def compute_and_save(name, low_resolution=True):
    """
    Low resolution matches neo better.
    Ideally we'd increase boozer resolution of Neo to see if that matches DESC, but
    Neo errors consistently.
    ...
    if low_resolution:
        # TODO: Make sure to also change resolution_requirement of effective ripple 3/2 in
        #  desc/compute/neoclassical from "tz" to "z" to avoid the grid being overridden
        #  to higher resolution.
        # By making that change, the dependencies of effective
        # ripple are computed at the higher oversampled resolution of
        # (M=eq.M_grid, N=eq.N_grid), but the FFTs that interpolate to the functions
        # to the quadrature nodes for the bounce integrals are done with eq.M_grid and
        # eq.N_grid points instead of double that.
...
def load_and_plot(name):
    ...
    plt.ylabel(r'$\epsilon_{\mathrm{eff}}^{3/2}$', fontsize=28)
